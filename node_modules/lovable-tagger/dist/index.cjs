...
function createTailwindConfigFeature() {
  ...
  return {
    ...
    async onBuildStart() {
      try {
        const tailwindInputFile = import_path.default.resolve(projectRoot, "./tailwind.config.ts");
        const tailwindIntermediateFile = import_path.default.resolve(projectRoot, "./.lov.tailwind.config.js");
        const tailwindJsonOutfile = import_path.default.resolve(projectRoot, "./src/tailwind.config.lov.json");
    ...
    onConfigureServer(server) {
      try {
        ...
        server.watcher.on("change", async (changedPath) => {
          if (import_path.default.normalize(changedPath) === import_path.default.normalize(tailwindInputFile)) {
            const tailwindIntermediateFile = import_path.default.resolve(projectRoot, "./.lov.tailwind.config.js");
            const tailwindJsonOutfile = import_path.default.resolve(projectRoot, "./src/tailwind.config.lov.json");
            await generateConfig(tailwindInputFile, tailwindIntermediateFile, tailwindJsonOutfile).catch((error) => {
              console.error("Error generating tailwind.config.lov.json:", error);
            });
          }
        });
...
function createVirtualOverridesFeature(context) {
  ...
  const normalizeFilePath = (filePath) => {
    ...
    return import_path2.default.resolve(projectRoot, filePath);
  };
  ...
  const invalidateModule = (filePath) => {
    if (!server)
      return;
    const normalizedPath = normalizeFilePath(filePath);
    const module2 = server.moduleGraph.getModuleById(normalizedPath);
    if (module2) {
      ...
      if (server.ws) {
        server.ws.send({
          ...
          updates: [
            {
              type: module2.type === "css" ? "css-update" : "js-update",
              path: module2.url,
  ...
  const broadcastOverrides = () => {
    if (!server?.ws)
      return;
    const overridesArray = Array.from(overrides.entries()).map(([path3, stored]) => ({
      path: path3,
      content: stored.content,
      sourceClientId: stored.sourceClientId
    }));
    log("Broadcasting overrides to all clients, count:", overridesArray.length);
    server.ws.send({
      type: "custom",
      event: "lovable:override-update",
      data: {
  ...
  const handleOverride = (data) => {
    ...
    log("Setting override for:", normalizedPath, "from client:", data.sourceClientId);
    overrides.set(normalizedPath, {
  ...
  const handleClearOverride = (data) => {
    const normalizedPath = normalizeFilePath(data.path);
    log("Clearing override for:", normalizedPath, "from client:", data.sourceClientId);
    overrides.delete(normalizedPath);
    invalidateModule(data.path);
    broadcastOverrides();
  };
  const handleClearAllOverrides = (data) => {
    log("Clearing all overrides, count:", overrides.size, "from client:", data?.sourceClientId);
  ...
  return {
    ...
    onConfigureServer(devServer) {
      ...
      devServer.ws.on("lovable:clear-all-overrides", async function(data) {
        try {
          await handleClearAllOverrides(data);
        } catch (error) {
    ...
    load(id) {
      const normalizedId = import_path2.default.isAbsolute(id) ? id : import_path2.default.resolve(projectRoot, id);
      ...
      if (overrideWithoutQuery !== void 0) {
        ...
        return overrideWithoutQuery.content;
      }
      return null;
    }
  };
