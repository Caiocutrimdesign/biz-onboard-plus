      ...
      <Card className={`relative overflow-hidden border-2 ${plan.popular ? 'border-primary shadow-xl' : 'border-border/50'} bg-card`}>
        ...
        <CardContent className="space-y-4">
          <Button 
            ...
            className={`w-full h-12 rounded-xl font-bold ${plan.popular ? 'bg-gradient-brand hover:opacity-90' : ''}`}
          >
            {expanded ? (
              <>
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="rounded-3xl bg-gradient-to-br from-muted/50 to-muted/20 p-8 border border-border/50"
      >
