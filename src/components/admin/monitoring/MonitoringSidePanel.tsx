    ...
    <div className="w-[400px] h-full bg-white flex flex-col border-r border-gray-100 shadow-[8px_0_32px_rgba(0,0,0,0.01)] relative z-20">
      ...
      <div className="p-8 border-b border-gray-50">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[10px] text-red-600 font-black uppercase tracking-[0.2em] mt-1">Telemetria em tempo real</p>
          </div>
          <div className="flex gap-2">
            <button className="p-2.5 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all border border-gray-100 text-gray-400">
        ...
        <div className="relative group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-gray-300 group-focus-within:text-red-600 transition-colors" />
          <Input 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Buscar veículo ou placa..." 
            className="pl-12 h-14 bg-gray-50/50 border-gray-100 text-gray-900 rounded-2xl focus:ring-red-50 focus:border-red-200 transition-all font-medium text-sm"
          />
        </div>
      </div>
      ...
      <div className="px-8 py-6 bg-gray-50/30">
        <div className="flex items-center justify-between mb-5">
          ...
          <button className="w-7 h-7 rounded-xl bg-red-600 text-white flex items-center justify-center shadow-lg shadow-red-100 hover:bg-red-700 transition-all hover:scale-110">
        ...
        <div className="space-y-4">
          <motion.div 
            whileHover={{ x: 4 }}
            className="p-5 bg-white rounded-[2rem] border border-red-50 shadow-sm flex items-center justify-between group hover:shadow-md transition-all cursor-pointer relative overflow-hidden"
          >
            <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-red-600 opacity-20" />
            <div>
              <p className="text-[9px] font-black text-red-500 uppercase tracking-widest mb-1.5 opacity-60">Operação Local</p>
              <p className="text-xs font-black text-gray-900 uppercase tracking-tight">Estoque Rastremix - Flamengo</p>
              <div className="flex gap-1.5 mt-3">
                {[1, 2, 3, 4].map(i => (
                   <div key={i} className="w-2.5 h-1 bg-red-100 rounded-full group-hover:bg-red-400 transition-colors" />
                ))}
              </div>
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 space-y-6 hidden-scrollbar">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-4">
             <div className="p-6 bg-red-50 rounded-[2.5rem] relative">
               <div className="absolute inset-0 bg-red-400/20 rounded-[2.5rem] blur-2xl animate-pulse" />
               <Activity className="w-10 h-10 text-red-600 relative z-10 animate-spin-slow" />
             </div>
             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-red-500/60">Interceptando sinais táticos...</p>
          </div>
          ...
          <motion.div 
            ...
            className="p-6 bg-white rounded-[2.5rem] border border-gray-50 shadow-sm hover:border-red-200 transition-all cursor-pointer group relative overflow-hidden"
          >
            ...
            <div className="pt-6 border-t border-gray-50 flex items-center justify-between">
               <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-gray-50 flex items-center justify-center group-hover:bg-red-50 transition-colors">
                    <MapPin className="w-4 h-4 text-gray-400 group-hover:text-red-500" />
                  </div>
               ...
               <button className="h-10 px-4 flex items-center gap-2 text-[10px] font-black text-red-600 uppercase tracking-widest hover:bg-red-50 rounded-xl transition-all">
        ...
        <div className="py-10">
          <button className="w-full h-16 bg-gray-50 text-gray-400 border border-transparent font-black rounded-3xl hover:bg-red-600 hover:text-white transition-all uppercase tracking-widest text-[10px] shadow-sm hover:shadow-xl hover:shadow-red-200">
