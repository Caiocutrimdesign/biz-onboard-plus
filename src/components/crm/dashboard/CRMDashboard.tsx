...
import { 
  ...
  ArrowDownRight, Calendar, Sparkles
} from 'lucide-react';
...
export default function CRMDashboard() {
  ...
  const revenueGrowth = analytics?.revenueLastMonth 
    ? ((analytics.revenue - analytics.revenueLastMonth) / analytics.revenueLastMonth) * 100 
    : 0;
  ...
  const totalLeads = leads.length || analytics?.totalLeads || 0;
  const conversionRate = analytics?.conversionRate || 0;
  const revenue = analytics?.revenue || analytics?.revenueThisMonth || 0;
  const averageDealValue = analytics?.averageDealValue || 0;
  const leadsBySource = analytics?.leadsBySource || {};
  const leadsByStatus = analytics?.leadsByStatus || {};

  const maxSourceCount = Object.values(leadsBySource).length > 0 
    ? Math.max(...Object.values(leadsBySource)) 
    : 1;
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          icon={TrendingUp}
          label="Ticket Médio"
          value={`R$ ${averageDealValue.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`}
          trend="up"
          trendValue="+8%"
              ...
              {recentLeads.length > 0 ? recentLeads.map((lead) => (
                ...
                >
                  <div className="w-12 h-12 rounded-full bg-gradient-brand flex items-center justify-center text-white font-bold">
                    {(lead.name || 'NA').split(' ').map(n => n[0]).join('').slice(0, 2)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 truncate">{lead.name || 'Sem nome'}</p>
                    <p className="text-sm text-gray-500 truncate">{lead.email || 'Sem email'}</p>
                  </div>
        ...
        <motion.div variants={itemVariants}>
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {Object.entries(leadsBySource).length > 0 ? Object.entries(leadsBySource).map(([source, count]) => (
                <div key={source} className="flex items-center gap-3">
                  <div className="flex-1">
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-gray-600">{getSourceLabel(source)}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={itemVariants}>
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {Object.entries(leadsByStatus).length > 0 ? Object.entries(leadsByStatus).map(([status, count]) => (
